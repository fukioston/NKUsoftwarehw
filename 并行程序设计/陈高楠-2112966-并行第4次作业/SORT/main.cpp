...
double sort_pthread(){
    ...
    cout << "sort_pthread方法耗时: " << time << "s" << endl;
    return time;
}
double arr_sort_static(int thread_num=8){
    ...
    QueryPerformanceCounter(&endTime);
    double time = (double)(endTime.QuadPart - beginTime.QuadPart) / (double)freq.QuadPart;
    cout << "arr_sort_static方法耗时: " << time << "s" << endl;
    return time;
}
...
double arr_sort_dynamic(int chunk=1,int thread_num=8){
    ...
    QueryPerformanceCounter(&endTime);
    double time = (double)(endTime.QuadPart - beginTime.QuadPart) / (double)freq.QuadPart;
//    cout << "arr_sort_dynamic方法耗时: " << time << "s" << endl;
    return time;
}
...
double arr_sort_guided(int chunk=1,int thread_num=8){
    ...
    QueryPerformanceCounter(&endTime);
    double time = (double)(endTime.QuadPart - beginTime.QuadPart) / (double)freq.QuadPart;
//    cout << "arr_sort_guided方法耗时: " << time << "s" << endl;
    return time;
}
...
//x++;
//}
//cout<<"分块为"<<chunk<<"时 "<<"guided t1:"<<t1/10<<endl;
//cout<<"分块为"<<chunk<<"时 "<<"dynamic t3:"<<t3/10<<endl;
//chunk+=5;
//
//}
...
cout<<"线程数为"<<num<<"时 "<<"guided t1:"<<t1/10<<endl;
cout<<"线程数为"<<num<<"时 "<<"dynamic t3:"<<t3/10<<endl;
num+=2;
